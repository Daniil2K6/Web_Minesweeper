(function() {
    ...
    const apiClient = {
        ...
        async saveScore(time, username, created_at) {
            try {
                const csrfToken = await this.getCSRFToken();
                const response = await fetch(`${API_BASE}/save-score`, {
                    ...
                    body: JSON.stringify({
                        time,
                        username: username || currentUser || null,
    ...
    function checkWin() {
        ...
        if (allSafeRevealed && gameActive) {
            ...
            renderBoard();
            // Активируем кнопку сохранения результата только для авторизованных пользователей
            setTimeout(() => {
                const saveResultBtn = document.getElementById('saveResultBtn');
                if (saveResultBtn && currentRole === 'user' && currentUser) {
                    console.log('✅ Кнопка активирована для', currentUser, 'время:', lastWinTime);
                    saveResultBtn.disabled = false;
                }
            }, 100);
                // Победа — НЕ сохраняем автоматически. Сохраняем по кнопке.
                lastWinTime = finalTime;
                const saveBtn = document.getElementById('saveResultBtn');
                if (saveBtn) saveBtn.disabled = false;
                console.log('Результат ожидает сохранения по кнопке. Время:', finalTime);
        }
    ...
    function resetGame() {
        ...
        // DEBUG: не отключаем кнопку сохранения — оставляем доступной для отладки
        const saveResultBtn = document.getElementById('saveResultBtn');
        if (saveResultBtn) {
            saveResultBtn.disabled = false;
        }
            ...
            html = `
                <div class="register-box">
                    ...
                    <div style="background:#2b1e3d; border-radius:40px; padding:15px; margin-bottom:16px; border:1px solid #9b79cf;">
                        Ты в игре, удачи!<br>После победы нажми <b>Сохранить результат</b>!
                    </div>
                    <button class="register-button" style="background:#352c4a;" id="logoutBtn">🚪 ВЫЙТИ</button>
                    <button class="register-button" style="background:#6fd96f; color:#222; margin-top:10px; display:none;" id="saveResultBtn">💾 Сохранить результат</button>
                </div>
                <div class="leaderboard">
                    <h3>🏆 ТВОЙ ТОП</h3>
                    <table class="leaderboard-table">
        ...
        setTimeout(() => {
            ...
            if (saveResultBtn) {
                saveResultBtn.addEventListener('click', async () => {
                    if (!lastWinTime) {
                        gameStatusEl.textContent = '⚠️ Нет результата для сохранения';
                        return;
                    }

                    // Получаем имя: сначала используем currentUser, иначе просим ввести
                    let username = currentUser;
                    if (!username) {
                        username = prompt('Введите имя для сохранения результата (отображается в лидерборде)');
                        if (!username) {
                            gameStatusEl.textContent = '⚠️ Имя не указано, сохранение отменено';
                            return;
                        }
                    ...
                    gameStatusEl.textContent = '⏳ Сохраняем результат...';
                    saveResultBtn.disabled = true;
                    try {
                        const result = await apiClient.saveScore(lastWinTime, username, new Date().toISOString());
    ...
    async function initializeApp() {
        ...
        // DEBUG: форсируем доступность кнопки сохранения для отладки
        try {
            const dbgBtn = document.getElementById('saveResultBtn');
            if (dbgBtn) {
                dbgBtn.disabled = false;
                console.log('DEBUG: saveResultBtn forced enabled');
            }
        } catch (e) {
            console.log('DEBUG: saveResultBtn not found on init');
        }
    }
    ...
    const saveResultBtn = document.getElementById('saveResultBtn');
    if (saveResultBtn) {
        saveResultBtn.addEventListener('click', async function(e) {
            e.preventDefault();
            console.log('💾 Нажата кнопка сохранения. Данные:', { lastWinTime, currentRole, currentUser });
            ...
            try {
                const result = await apiClient.saveScore(lastWinTime, username, new Date().toISOString());
                console.log('✅ Результат сохранён:', result);
                gameStatusEl.textContent = '✅ Результат сохранён! Время: ' + lastWinTime.toFixed(3) + 'с';
            } catch (error) {
                console.error('❌ Ошибка сохранения результата:', error);
                gameStatusEl.textContent = '⚠️ Ошибка: ' + (error.message || 'Не удалось сохранить');
                this.disabled = false;
            }
        });
    }
